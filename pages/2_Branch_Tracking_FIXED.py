...
# Automatically find the most recent bundle analysis file
def find_bundle_file():
    """Find the most recent COMPLETE_BUNDLE_ANALYSIS CSV file"""
    pattern = os.path.join(DATA_DIR, "COMPLETE_BUNDLE_ANALYSIS*.csv")
    files = glob.glob(pattern)
    if files:
        # Sort by filename (includes timestamp) and return the most recent
...
if not BUNDLE_FILE or not os.path.exists(BUNDLE_FILE):
    st.error("❌ Bundle data not found. Please upload a COMPLETE_BUNDLE_ANALYSIS CSV file to the data/ folder.")
    st.stop()

# Show which file is being used (for transparency)
st.caption(f"📊 Using: {os.path.basename(BUNDLE_FILE)}")
...
if os.path.exists(SALES_LOG):
    ...
    # Convert timestamp immediately after loading
...
# This week's sales - now timestamp is already converted
if len(branch_sales) > 0:
    ...
    week_revenue = this_week_sales['revenue_estimate'].sum()
else:
    week_bundles = 0
    week_revenue = 0
...
# Tabs
tab1, tab2 = st.tabs(["ðŸ“¦ Available Bundles", "ðŸ“Š My Performance"])
...
with tab1:
    ...
    for idx, row in top_bundles.iterrows():
        ...
        with col1:
            ...
            revenue = int(row[REVENUE_COL]) if REVENUE_COL else 0
            
            st.markdown(f"""
            **{part1} + {part2}**  
            {customers} customers â€¢ {confidence:.1f}% confidence â€¢ ${revenue:,} revenue
            """)
        ...
        with col2:
            ...
            if st.button("Mark as Sold", key=button_key, use_container_width=True):
                ...
                new_sale = {
                    ...
                    'customers': row[CUSTOMERS_COL] if CUSTOMERS_COL else 0,
                    'confidence': row[CONFIDENCE_COL] if CONFIDENCE_COL else 0,
                    'revenue_estimate': row[REVENUE_COL] if REVENUE_COL else 0,
                    'status': 'Sold'
                }
                ...
                st.success(f"âœ… Bundle marked as sold!")
                st.rerun()
