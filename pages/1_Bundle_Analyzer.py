...
with st.sidebar:
    ...
    if st.button("ðŸ  Home", use_container_width=True):
        st.session_state.admin_authenticated = False
        st.switch_page("Home.py")
    if st.button("ðŸ“ˆ Admin Dashboard", use_container_width=True):
...
if BUNDLE_FILE and os.path.exists(BUNDLE_FILE):
    ...
    st.info(f"📊 Using data file: {os.path.basename(BUNDLE_FILE)}")
    ...
    tab1, tab2, tab3 = st.tabs(["ðŸ“Š Overview", "ðŸ“‹ Data Table", "âš™ï¸ Settings"])
