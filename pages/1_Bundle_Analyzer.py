...
if os.path.exists(BUNDLE_FILE):
    ...
    with col2:
        if 'Customer_Base_Conf' in df.columns:
            total_customers = df['Customer_Base_Conf'].sum()
            st.markdown(f"""
            <div class="metric-card">
    ...
    with col3:
        if 'Enhanced_Confidenci' in df.columns:
            avg_confidence = df['Enhanced_Confidenci'].mean()
            st.markdown(f"""
            <div class="metric-card">
    ...
    with col4:
        if 'Annual_Revenue_Pote' in df.columns:
            total_revenue = df['Annual_Revenue_Pote'].sum()
            st.markdown(f"""
            <div class="metric-card">
    ...
    with tab1:
        ...
        if 'Enhanced_Confidenci' in df.columns:
            ...
            confidence_bins = pd.cut(df['Enhanced_Confidenci'], bins=[0, 50, 70, 80, 90, 100], 
        ...
        top_bundles = df.nlargest(20, 'Enhanced_Confidenci')[['Part_1', 'Part_2', 'Customer_Base_Conf', 'Enhanced_Confidenci', 'Annual_Revenue_Pote']]
    ...
    with tab2:
        ...
        # Filter data
        filtered_df = df[
            (df['Enhanced_Confidenci'] >= min_confidence) & 
            (df['Customer_Base_Conf'] >= min_customers)
        ]
